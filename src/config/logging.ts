import { Encoder, ENCODER_TYPES, Handler, ImLoggerConfig, LOG_LEVELS, SINK_TYPES, SinkType } from '@imtbl/imlogging';
import * as dotenv from 'dotenv';

import { getEnv, validateString } from '../library/utils';
...
function getHandlers(): Handler[] {
    ...
    const handlers: Handler[] = names.split(',').map((name) => {
        const type = validateString<SinkType>(getEnv(`LOG_HANDLER_${name}_TYPE`), SINK_TYPES);
        const encoder = validateString<Encoder>(getEnv(`LOG_HANDLER_${name}_ENCODER`), ENCODER_TYPES);
        if (type === 'console') {
            return {
                type,
                encoder
